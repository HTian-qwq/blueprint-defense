(function(root){
  ...
  class Actors{
    constructor(manifest){
      this.manifest=manifest;this.images=new Map();
      this.ready=Promise.all(Object.values(manifest.animations).flatMap(c=>c.pages).map(p=>new Promise((resolve,reject)=>{
    ...
    draw(ctx,t,time,p,cell,footprint){
      const state=pose(this.manifest,t,time),{clip,page,index}=state,img=this.images.get(page.file);if(!img)return false;
