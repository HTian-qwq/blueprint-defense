...
with subprocess.Popen([sys.executable,'-u',str(ROOT/'tools/serve.py'),'--port','0'],cwd=ROOT,env=env,stdout=subprocess.PIPE,stderr=subprocess.DEVNULL,text=True,encoding='utf-8') as server:
    try:
        ...
        run=subprocess.run([NODE,str(ROOT/'tests/web_browser.test.cjs')],cwd=ROOT,env={**env,'BLUEPRINT_WEB_URL':url.group()},capture_output=True,text=True,encoding='utf-8',timeout=180)
        results.append({'suite':'web_browser','returncode':run.returncode,'output':run.stdout+run.stderr})
        print('web_browser','PASS' if run.returncode==0 else 'FAIL',flush=True)
...
report={'version':'0.18.1','passed':sum(int(re.search(r'"passed":\s*(\d+)',r['output']).group(1)) for r in results if r['returncode']==0 and r['suite']!='belt_assets'),'beltOrientations':12 if belts.returncode==0 else 0,'suites':results,'buildSha256':hashlib.sha256((ROOT/'dist/offline/blueprint_defense.html').read_bytes()).hexdigest()}
