...
report={'version':'0.18','passed':sum(int(re.search(r'"passed":\s*(\d+)',r['output']).group(1)) for r in results if r['returncode']==0 and r['suite']!='belt_assets'),'beltOrientations':12 if belts.returncode==0 else 0,'suites':results,'buildSha256':hashlib.sha256((ROOT/'dist/offline/blueprint_defense.html').read_bytes()).hexdigest()}
